...
class Breakthrough():
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    print((self.__Deck.GetNumberOfCards()))
                    ...
                    if MenuChoice == "D":
                        ...
                        if self.__CurrentLock.GetPeakUsed() == True:
                            print("You cannot use that twice!")
                        else:
                            self.__CurrentLock.SetPeakUsed()
                            print("")
                            print(f"{self.__Hand.GetCardDescriptionAt(0)}, {self.__Hand.GetCardDescriptionAt(1)}, {self.__Hand.GetCardDescriptionAt(2)}")
                            print("")  
                    elif MenuChoice == "M":
                        if self.__MulliganUsed == False:
                            Count = 0
                            while Count < 5 and self.__Deck.GetNumberOfCards() > 0:
                                CardToMove = self.__Deck.RemoveCard(self.__Deck.GetCardNumberAt(0))
                                self.__Discard.AddCard(CardToMove)
                                Count += 1
                        ...
                        self.__Score += 1
                        print(f"Your Final Score is {self.__Score}!")
                        return 1
    ...
    def __PlayCardToSequence(self, CardChoice):
        if self.__Sequence.GetNumberOfCards() > 0:
            if self.__Hand.GetCardDescriptionAt(CardChoice - 1)[0] != self.__Sequence.GetCardDescriptionAt(self.__Sequence.GetNumberOfCards() - 1)[0]:
                ...
                print(f"You cannot play two of the same card types sequentially, {self.__Hand.GetCardDescriptionAt(CardChoice - 1)[0]} is the card type you have played twice.")
        else:
            self.__Score += self.__MoveCard(self.__Hand, self.__Sequence, self.__Hand.GetCardNumberAt(CardChoice - 1))
            self.__GetCardFromDeck(CardChoice)
    ...
    def __GetChoice(self):
        print()
        if self.__MulliganUsed == True:
            Choice = input("(D)iscard inspect, (U)se card, (P)eek, (Q)uit :> ").upper()
        else:
            Choice = input("(D)iscard inspect, (U)se card, (P)eek, (M)ulligan, (Q)uit :> ").upper()
        return Choice
...
class Lock():
    def __init__(self):
        self._Challenges = []
        self._UsedPeak = False
    
    def GetPeakUsed(self):
        return self._UsedPeak

    def SetPeakUsed(self):
        self._UsedPeak = True
...
"""    def Shuffle(self):
        midpoint = len(self._Cards) / 2
        RightDeck = [] * (len(self._Cards) / 2)
        LeftDeck = [] * (len(self._Cards) / 2)
        for i in range(0,midpoint):
            LeftDeck[i] = self._Cards[midpoint+i]
            RightDeck[i] = self._Cards[i]
            finished = False
            Count = 0
            LeftCount = 0
            RightCount = 0
            while finished == False:    
                for i in range(0,1):
                    ...
                    for y in range(0,random_number):
                        if i == 0:
                            self._Cards[Count] = LeftDeck[LeftCount + random_number]
                            LeftCount += 1
                        if i == 1:
                            self._Cards[Count] = RightDeck[RightCount + random_number]
                            RightCount += 1
                        Count +=1
        """
