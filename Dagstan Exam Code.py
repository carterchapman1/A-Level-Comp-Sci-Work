...
class Dastan:
    ...
    def ProcessBhukampa(self):
        ...
        for i in range(0,5):
            ...
            temp = self._Board[index1]
            self._Board[index1] = self._Board[index2]
            self._Board[index2] = temp

    def __UseMoveOptionOffer(self):
        ReplaceChoice = int(input("Choose the move option from your queue to replace (1 to 5): "))
        while not(ReplaceChoice < 5 and ReplaceChoice > 1):
            ReplaceChoice = int(input("Choose the move option from your queue to replace (1 to 5) cos u got it wrong : "))
